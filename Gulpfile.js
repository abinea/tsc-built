...
const {
  ...
  refRwcBaseline,
  localRwcBaseline,
} = require("./scripts/build/tests");
const {
  buildProject,
  cleanProject,
  watchProject,
} = require("./scripts/build/projects");
...
const libs = libraries.libs.map((lib) => {
  const relativeSources = ["header.d.ts"].concat(
    (libraries.sources && libraries.sources[lib]) || [lib + ".d.ts"]
  );
...
const generateLibs = () => {
  return merge2(
    libs.map(({ sources, target, relativeTarget }) =>
...
const generateDiagnostics = async () => {
  if (
    needsUpdate(diagnosticMessagesJson, [
...
task("generate-diagnostics").description =
  "Generates a diagnostic file in TypeScript based on an input JSON file";

const cleanDiagnostics = () =>
  del([diagnosticInformationMapTs, diagnosticMessagesGeneratedJson]);
cleanTasks.push(cleanDiagnostics);

const watchDiagnostics = () =>
  watch(["src/compiler/diagnosticMessages.json"], task("generate-diagnostics"));
...
const lkgPreBuild = parallel(
  generateLibs,
  series(buildScripts, generateDiagnostics, buildShimsAndTools)
);
...
task(
  "watch-tsc",
  series(lkgPreBuild, parallel(watchLib, watchDiagnostics, watchTsc))
);
task("watch-tsc").description =
  "Watch for changes and rebuild the command-line compiler only.";

// Pre-build steps when targeting the built/local compiler.
const localPreBuild = parallel(
  generateLibs,
  series(buildScripts, generateDiagnostics, buildShimsAndTools, buildTsc)
);
...
const buildServices = (() => {
  ...
  const buildTypescriptServicesOut = () =>
    buildProject("src/typescriptServices/tsconfig.json", cmdLineOptions);

  // create typescriptServices.js
  const createTypescriptServicesJs = () =>
    src("built/local/typescriptServices.out.js")
  ...
  const createTypescriptServicesDts = () =>
    src("built/local/typescriptServices.out.d.ts")
  ...
  const createTypescriptJs = () =>
    src("built/local/typescriptServices.js")
  ...
  const createTypescriptDts = () =>
    src("built/local/typescriptServices.d.ts")
  ...
  const createTypescriptStandaloneDts = () =>
    src("built/local/typescriptServices.d.ts")
      .pipe(newer("built/local/typescript_standalone.d.ts"))
      .pipe(
        transform((content) =>
          content.replace(
            /declare (namespace|module) ts/g,
...
task(
  "watch-services",
  series(preBuild, parallel(watchLib, watchDiagnostics, watchServices))
);
task("watch-services").description =
  "Watches for changes and rebuild language service only";
...
task(
  "watch-tsserver",
  series(preBuild, parallel(watchLib, watchDiagnostics, watchServer))
);
task("watch-tsserver").description =
  "Watch for changes and rebuild the language server only";
...
task(
  "watch-min",
  series(preBuild, parallel(watchLib, watchDiagnostics, watchTsc, watchServer))
);
task("watch-min").description =
  "Watches for changes to a tsc and tsserver only";
...
const buildLssl = (() => {
  ...
  const buildServerLibraryOut = () =>
    buildProject("src/tsserverlibrary/tsconfig.json", cmdLineOptions);

  // create tsserverlibrary.js
  const createServerLibraryJs = () =>
    src("built/local/tsserverlibrary.out.js")
  ...
  const createServerLibraryDts = () =>
    src("built/local/tsserverlibrary.out.d.ts")
...
task("clean-lssl").description =
  "Clean outputs for the language service server library";
...
task(
  "watch-lssl",
  series(preBuild, parallel(watchLib, watchDiagnostics, watchLssl))
);
task("watch-lssl").description =
  "Watch for changes and rebuild tsserverlibrary only";
...
task("clean-tests").description =
  "Cleans the outputs for the test infrastructure";
...
task("clean-eslint-rules").description =
  "Cleans the outputs for the eslint rules";

const runEslintRulesTests = () =>
  runConsoleTests(
    "scripts/eslint/built/tests",
...
const generateTypesMap = () =>
  src("src/server/typesMap.json")
...
const copyBuiltLocalDiagnosticMessages = () =>
  src(diagnosticMessagesGeneratedJson)
    .pipe(newer(builtLocalDiagnosticMessagesGeneratedJson))
    .pipe(dest("built/local"));

const cleanBuiltLocalDiagnosticMessages = () =>
  del(builtLocalDiagnosticMessagesGeneratedJson);
cleanTasks.push(cleanBuiltLocalDiagnosticMessages);

const buildOtherOutputs = parallel(
  buildCancellationToken,
  buildTypingsInstaller,
  buildWatchGuard,
  generateTypesMap,
  copyBuiltLocalDiagnosticMessages
);
task("other-outputs", series(preBuild, buildOtherOutputs));
task("other-outputs").description =
...
task("runtests").flags = {
  ...
  "   --light":
    "Run tests in light mode (fewer verifications, but tests run faster)",
  "   --dirty": "Run tests without first cleaning test output directories",
  "   --stackTraceLimit=<limit>":
    "Sets the maximum number of stack frames to display. Use 'full' to show all frames.",
...
task(
  "runtests-parallel",
  series(preBuild, preTest, runTestsParallel, postTest)
);
task("runtests-parallel").description =
...
task("test-browser-integration", () =>
  exec(process.execPath, ["scripts/browserIntegrationTest.js"])
);
task("test-browser-integration").description =
  "Runs scripts/browserIntegrationTest.ts which tests that typescript.js loads in a browser";
...
task("diff-rwc", () =>
  exec(getDiffTool(), [refRwcBaseline, localRwcBaseline], {
    ignoreExitCode: true,
    waitForExit: false,
  })
);
task("diff-rwc").description =
 ...
 */
const baselineAccept = (localBaseline, refBaseline) =>
  merge2(
    src([`${localBaseline}/**`, `!${localBaseline}/**/*.delete`], {
      base: localBaseline,
...
task("baseline-accept").description =
  "Makes the most recent test results the new baseline, overwriting the old baseline";

task("baseline-accept-rwc", () =>
  baselineAccept(localRwcBaseline, refRwcBaseline)
);
task("baseline-accept-rwc").description =
  "Makes the most recent rwc test results the new baseline, overwriting the old baseline";

const buildLoggedIO = () =>
  buildProject("src/loggedIO/tsconfig-tsc-instrumented.json");
...
const tscInstrumented = () =>
  exec(process.execPath, [
    "built/local/instrumenter.js",
    "record",
    cmdLineOptions.tests || "iocapture",
    "built/local/tsc.js",
  ]);
task(
  "tsc-instrumented",
...
const updateSublime = () =>
  src(["built/local/tsserver.js", "built/local/tsserver.js.map"]).pipe(
    dest("../TypeScript-Sublime-Plugin/tsserver/")
  );
task("update-sublime", updateSublime);
task("update-sublime").description = "Updates the sublime plugin's tsserver";

const buildImportDefinitelyTypedTests = () =>
  buildProject("scripts/importDefinitelyTypedTests");
const cleanImportDefinitelyTypedTests = () =>
  cleanProject("scripts/importDefinitelyTypedTests");
...
const importDefinitelyTypedTests = () =>
  exec(process.execPath, [
...
task(
  ...
  series(buildImportDefinitelyTypedTests, importDefinitelyTypedTests)
);
task("importDefinitelyTypedTests").description =
  "Runs the importDefinitelyTypedTests script to copy DT's tests to the TS-internal RWC tests";
...
task("configure-insiders").description =
  "Runs scripts/configurePrerelease.ts to prepare a build for insiders publishing";

const configureExperimental = () =>
  exec(process.execPath, [
    "scripts/configurePrerelease.js",
    "experimental",
    "package.json",
...
task("configure-experimental").description =
  "Runs scripts/configurePrerelease.ts to prepare a build for experimental publishing";

const createLanguageServicesBuild = () =>
  exec(process.execPath, ["scripts/createLanguageServicesBuild.js"]);
task(
  "create-language-services-build",
  series(buildScripts, createLanguageServicesBuild)
);
task("create-language-services-build").description =
  "Runs scripts/createLanguageServicesBuild.ts to prepare a build which only has the require('typescript') JS.";
...
task(
  ...
  series(
    ...
    task("LKG"),
    task("clean"),
    task("runtests-parallel"),
    publishNightly
  )
);
task("publish-nightly").description =
...
const watchRuntests = () =>
  watch(
    ["built/local/*.js", "tests/cases/**/*.ts", "tests/cases/**/tsconfig.json"],
    { delay: 5000 },
    async () => {
...
task("watch").description =
  "Watches for changes and rebuilds and runs tests in parallel.";
task("watch").flags = {
  "-t --tests=<regex>":
    "Pattern for tests to run. Forces tests to be run in a single worker.",
  "   --failed":
    "Runs tests listed in '.failed-tests'. Forces tests to be run in a single worker.",
  "-r --reporter=<reporter>": "The mocha reporter to use.",
  "   --keepFailed": "Keep tests in .failed-tests even if they pass",
  "   --light":
    "Run tests in light mode (fewer verifications, but tests run faster)",
...
task("help", () =>
  exec("gulp", ["--tasks", "--depth", "1", "--sort-tasks"], {
    hidePrompt: true,
  })
);
